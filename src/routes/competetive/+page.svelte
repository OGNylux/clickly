<script lang="ts">
    ...
    import { crops, emojis, isClassic, score, unlockedClicker, unlockedFarmItems, unlockedPassiveItems, user } from '$lib/store';
    ...
    onMount(async() => {
        ...
        socket.onmessage = (event) => {
            ...
            if (m.type == "gameState") {
                ...
                console.log("Received game state", m.message.rest);
                let match = m.message.match(/emojis:\s*(\S*),/);
                if (match)
                    emojis.set(match[1]);
        ...
        saveInterval = setInterval(() => {
            ...
            const message: ClientMessage = {
                ...
                message: {
                    rest: `emojis: ${emojis.get()},crops: ${crops.get()},clicker: ${unlockedClicker.get().getAmount()},passive: ${arr},farm: ${unlockedFarmItems.get()}`,
                    score: score.get()
                }
            };
